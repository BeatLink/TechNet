# VSCodium ###########################################################################################################################################
#
# The editor, its settings and its extension set. Two instances read this: Odin's own, and the one Heimdall runs for Thor over waypipe. Both are
# declared here rather than beside either host, because two copies of an editor's configuration drift apart.
#
# settings.json is a symlink into the store, so nothing set from the settings UI survives -- everything wanted has to be here. What follows was
# reconciled against settings.json.hmbackup, the mutable file home-manager displaced when it took the path over.
#
# The package itself is left to whoever enables this. Odin takes it from home-manager below; Heimdall also puts it on the system PATH, because a
# non-login ssh session is where its launches arrive.
#
{
    config,
    lib,
...
let
    cfg = config.technet.vscodium;
in
{
    options.technet.vscodium = {
        enable = lib.mkEnableOption "the shared VSCodium configuration";

        userSettings = lib.mkOption {
            type = lib.types.attrsOf lib.types.anything;
            description = "Editor settings shared by every VSCodium instance in the TechNet, written as a profile's userSettings.";
        };
    };
    ...
    config = lib.mkMerge [
        # Settings -----------------------------------------------------------------------------------------------------------------------------------
        {
            technet.vscodium.userSettings = {
                ...
                # sops -------------------------------------------------------------------------------------------------------------------------------
                "sops.defaults.ageKeyFile" = "/home/beatlink/.config/sops/age/keys.txt";
        ...
        # Editor -------------------------------------------------------------------------------------------------------------------------------------
        (lib.mkIf cfg.enable {
            # home-manager.useGlobalPkgs is enabled in nix/0-common/2-users, so the extension overlay is applied to the system package set rather than
            # a per-user one. allowUnfree is already set globally in nix/0-common/1-system/5-software/4-unfree-packages.nix, which covers the unfree
            # extensions here.
