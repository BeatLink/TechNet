...
{ config, ... }:
{
    disko.devices = {
        disk.data-drive = {
            ...
            content = {
                ...
                partitions = {
                    zfs-data-partition = {
                        # This creates a ZFS partition to be added to the ZFS pool
                        size = "100%";
                        content = {
                            type = "zfs";
        ...
        zpool."data-pool-${config.networking.hostName}" = {
            type = "zpool";
            options = {
                autotrim = "on";
            };
            ...
            postCreateHook = ''
                zfs set keylocation="prompt" "data-pool-${config.networking.hostName}";             # use this to read the key during boot
            '';
            datasets = {
                storage = {
    ...
    systemd.tmpfiles.rules = [
        "d /Storage 1777 beatlink beatlink" # Creates the mount point and sets needed permissions
    ];
}
