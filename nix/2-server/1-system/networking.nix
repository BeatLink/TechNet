...
#
# The initrd wireguard key handling and the tunnel recovery loop come from the shared module in 0-common; what is left here is
# Heimdall-specific -- its identity, the NAT/forwarding that makes it the relay, and the peer table for the whole TechNet.
#
...
}:
{
    technet.initrdWireguard = {
        ...
        probe.address = "192.168.0.1";
        reconfigureLinks = [
            "enp2s0f1"
            "wireguard0"
        ];
        kernelModules = [ "r8169" ]; # Ethernet NIC driver
    };
    ...
    boot.kernel.sysctl."net.ipv4.conf.all.forwarding" = true; # Enables routing between peers for wireguard
